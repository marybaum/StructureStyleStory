	...
	function widget($args, $instance) {
		...
		if ( $r->have_posts() ) {
			...
			while ( $r->have_posts()) {
				...
				echo '<li>' . '<h2> <a href="' . get_permalink() . '">' . woocommerce_get_template( 'single-product/title.php' ) . '</h2> </a> ' . ( has_post_thumbnail() ? get_the_post_thumbnail( $r->post->ID, 'shop_thumbnail' ) : woocommerce_placeholder_img( 'shop_thumbnail' ) )
